...
% hatches — a beamer->article shim layer as one ordinary preamble.  LaTeXML
% 0.8.8 parses it directly, and it also compiles under plain pdflatex (which
% BookML's build relies on: the PDF pass runs first and doubles as a
% regression check).
%
...
% the accessible figure path (labels stay glyphs, not live DOM text).
...
% Alt-text bridge: \kitfigdesc attaches a figure description to the generated
% image on the web target (LaTeXML binding); no-op under pdflatex.
\usepackage{shared/kitalt}
% Overlay bridge: LaTeXML re-lets \item at every list start, past the \item
% wrapper below, so \item<2-> specs are stripped on the LaTeXML side
% (shared/kitoverlay.sty.ltxml); no-op under pdflatex.
\usepackage{shared/kitoverlay}
...
% silently lost.  Harmless under pdflatex.
...
% literal TeX), and a circled digit conveys nothing a
...
% --- \fig{path}{alt-key} -----------------------------------------------------
% The TikZ source is \input (BookML images it via dvisvgm); \kitfigdesc runs
% AFTER the input so the generated image node exists, and attaches
% \figalt{#2} to it as the <img alt> (see shared/kitalt.sty[.ltxml]).
\newcommand{\fig}[2]{%
  \par\begin{center}\input{#1}\kitfigdesc{\figalt{#2}}\end{center}\par}
% \figw{width}{path}{key}: the width cap is a SLIDES-layout concern; on the
% web the image flows at page width, so the width argument is ignored here.
\newcommand{\figw}[3]{\fig{#2}{#3}}
